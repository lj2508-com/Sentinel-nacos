...
public class AuthorityRuleControllerV2 {
    ...
    // V2 改造：使用 RuleRepository 存储规则的内存副本
    @Autowired
    ...
    /**
     * V2 改造：查询应用的全部规则，不再需要ip和port
     */
    @GetMapping("/rules")
    @AuthAction(PrivilegeType.READ_RULE)
    public Result<List<AuthorityRuleEntity>> apiQueryAllRulesForApp(@RequestParam String app) {
    ...
    public Result<AuthorityRuleEntity> apiAddAuthorityRule(@RequestBody AuthorityRuleEntity entity) {
        ...
        try {
            ...
            // V2 改造：调用 publishRules 发布到Nacos
    ...
    @PutMapping("/rule/{id}")
    @AuthAction(PrivilegeType.WRITE_RULE)
    public Result<AuthorityRuleEntity> apiUpdateAuthorityRule(@PathVariable("id") Long id,
        ...
        entity.setId(id);
        Date date = new Date();
        entity.setGmtCreate(oldEntity.getGmtCreate()); // 创建时间保持不变
        entity.setGmtModified(date);
        try {
            entity = repository.save(entity);
            if (entity == null) {
